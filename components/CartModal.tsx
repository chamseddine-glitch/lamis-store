...
export const CartModal: React.FC<{ isOpen: boolean; onClose: () => void; }> = ({ isOpen, onClose }) => {
    ...
    const deliveryFee = useMemo(() => {
        ...
        const feeForWilaya = settings.deliveryFees.find(f => f.wilaya === selectedWilaya);
        if (!feeForWilaya) return deliveryType === 'home' ? 500 : 300; // Fallback
        return deliveryType === 'home' ? feeForWilaya.home : feeForWilaya.office;
    }, [selectedWilaya, deliveryType, settings.deliveryFees]);
    ...
    const handleCheckoutSubmit = async (e: React.FormEvent) => {
        ...
        if (settings.deliveryCompanies?.length > 0) {
            ...
            {cart.length === 0 ? (
                ...
                <>
                        ...
                        {cart.map(item => (
                            <div key={item.id} className="flex items-start gap-4 p-2 bg-gray-50/50 dark:bg-slate-800/20 rounded-lg animate-fade-in-up">
                                ...
                                <div className="text-left flex flex-col items-end">
                                    ...
                                    <button onClick={() => handleRemoveItem(item.id)} className="text-red-500 hover:text-red-700 text-xs mt-1 flex items-center gap-1 p-1 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors">
                    ...
                    <div className="border-t dark:border-slate-700 pt-4 mt-4">
                        ...
                        <p className="text-sm text-text-muted dark:text-slate-400 mt-1">سيتم إضافة رسوم التوصيل عند الدفع.</p>
                        <div className="flex gap-4 mt-6">
                            <button onClick={handleClose} type="button" className="flex-1 bg-gray-200 text-text-base dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 px-6 py-3 rounded-lg font-bold hover:bg-gray-300 transition-all active:scale-95">
                                متابعة التسوق
                            </button>
                            <button onClick={() => setView('checkout')} type="button" className="flex-1 bg-secondary text-white px-6 py-3 rounded-lg font-bold hover:bg-opacity-90 transition-all active:scale-95 shadow-lg shadow-secondary/30 hover:shadow-secondary/50 transform hover:-translate-y-0.5">
                                إتمام الطلب
                            </button>
                        </div>
                    </div>
    ...
    const renderCheckoutView = () => (
        <form onSubmit={handleCheckoutSubmit} className="space-y-4">
            ...
            {settings.deliveryCompanies?.length > 0 && (
                 <div className="animate-fade-in-up">
                    <select value={deliveryCompany} onChange={e => setDeliveryCompany(e.target.value)} className={inputStyle} required>
                        ...
                        {settings.deliveryCompanies.map(company => <option key={company} value={company}>{company}</option>)}
                    </select>
                 </div>
            )}
            ...
            <div className="flex gap-4 mt-6">
                    <button type="button" onClick={() => setView('cart')} className="flex-1 bg-gray-200 text-text-base dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 px-6 py-3 rounded-lg font-bold hover:bg-gray-300 transition-all active:scale-95" disabled={isSubmitting}>
                    العودة للسلة
                </button>
    ...
    return (
        <div className={`fixed inset-0 bg-black z-50 flex justify-center items-center p-4 transition-opacity duration-300 ${isClosing ? 'bg-opacity-0' : 'bg-opacity-50'}`}>
            <div className={`bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col transition-all duration-300 ${isClosing ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}>
                <div className="p-6 pb-2 flex-shrink-0 relative">
                    <button onClick={handleClose} className="absolute top-4 left-4 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white z-10">
